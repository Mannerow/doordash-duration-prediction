...
def decode_dataframe(dv, df):
    ...
    for col in original_columns:
        ...
        relevant_columns = [feature for feature in df_dense.columns if feature.startswith(col + "=")]
        if relevant_columns:
            # Combine the one-hot encoded columns back into a single categorical column
            original_df[col] = df_dense[relevant_columns].idxmax(axis=1).apply(lambda x: x.split("=")[-1])
